...
async function homeContent() {
  try {
    const url = `http://www.muoupan.top/`;
...
async function searchContent(keyword) {
    try {
        ...
        const url = `http://www.muoupan.top/index.php/vod/search.html?wd=${encodedKeyword}`;
...
async function categoryContent(tid, pg = 1, extend) {
  try {
    ...
    let url = `http://www.muoupan.top/index.php/vod/show/area/{area}/class/{class}/by/{by}/id/${tid}/year/{year}/page/${pg}.html`;
    // 替换 URL 中的占位符
    if (extendObj) {
...
async function detailContent(ids) {
  const url = `http://www.muoupan.top${ids}`;
  try {
    //console.log(url);
    const html = await 访问网页(url);
    // 使用正则表达式提取信息
